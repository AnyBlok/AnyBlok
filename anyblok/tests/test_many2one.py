...
from anyblok import Declarations, ForeignKeyConstraint
...
from .conftest import init_registry, init_registry_with_bloks, reset_db
...
def registry_many2one(request, bloks_loaded, db_schema):
    reset_db()
    registry = init_registry_with_bloks([], request.param[0])
...
class TestMany2One:
    ...
    def test_expire_field(self, registry_many2one):
        ...
        id_of_address = registry.expire_attributes["Model.Person"][column_name]
...
class TestMany2OneAutoDetectColumn:
    ...
    def test_check_autodetect_type(self, registry_many2one_auto_detect):
        ...
        assert str(registry.Address.id.property.columns[0].type) == str(
            registry.Person.address_id.property.columns[0].type
        )
...
def _many2one_with_same_name_for_column_names(**kwargs):
    ...
    class Person:
        name = String(primary_key=True)
        address = Many2One(model=Model.Address, column_names="address")
    ...
    def test_with_multi_foreign_key_on_unexisting_named_column2(self):
        def add_in_registry():
            ...
            class Test2:
                ...
                test = Many2One(
                    model=Model.Test,
                    column_names=("other_test_id", "other_test_id2"),
                )
