...
from anyblok.testing import LogCapture, TestCase
...
class TestRegistry:
    ...
    def test_reload(self):
        registry = self.init_registry(None)
        bloks_before_reload = (
            registry.System.Blok.query("name")
            .filter(registry.System.Blok.state == "installed")
            .all()
        )
        registry.reload()
        bloks_after_reload = (
    ...
    def test_load_blok(self):
        ...
        for entry in RegistryManager.declared_entries:
            RegistryManager.loaded_bloks["blok"][entry] = {"registry_names": []}
...
class TestRegistry2:
    ...
    def define_cls(
        self, typename="Model", name="Test", val=1, usesuper=False, inherit=None
    ):
        from anyblok import Declarations
    ...
    def test_postcommit_hook_call_only_if_raised(self):
        ...
        registry = self.init_registry(add_in_registry)
        registry.Test.postcommit_hook("_postcommit_hook", call_only_if="raised")
        assert do_somthing == 0
        registry.commit()
        assert do_somthing == 0
        registry.Test.precommit_hook("_precommit_hook")
    ...
    def test_postcommit_hook_call_only_if_always(self):
        ...
        registry = self.init_registry(add_in_registry)
        registry.Test.postcommit_hook("_postcommit_hook", call_only_if="always")
        assert do_somthing == 0
        registry.commit()
        assert do_somthing == 1
        registry.Test.precommit_hook("_precommit_hook")
...
class TestRegistry3:
    ...
    def test_get(self, registry_blok):
        registry = registry_blok
        assert registry.get("Model.System.Blok") is registry.System.Blok
    ...
    def test_apply_state_on_an_unexisting_blok(self, registry_blok):
        registry = registry_blok
        with pytest.raises(RegistryException):
            registry.apply_state("unexisting_blok", "toinstall", ["toinstall"])
