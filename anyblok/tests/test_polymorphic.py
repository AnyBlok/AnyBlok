...
from anyblok.column import Date, DateTime, Integer, String
...
def check_registry(Model, **kwargs):
    ...
    t2 = (
        Model.query()
        .filter(Model.type_entity == Model.__mapper__.polymorphic_identity)
        .first()
    )
    assert t2 is t
    return t
...
class TestMultiTablePolyMixins:
    ...
    def test_multi_table_poly_mixins(self, registry_multi_table_poly_mixins):
        ...
        check_registry(registry.Engineer, engineer_name="An engineer")
        check_registry(
            registry.Manager, manager_name="An manager", birthday=date.today()
        )
